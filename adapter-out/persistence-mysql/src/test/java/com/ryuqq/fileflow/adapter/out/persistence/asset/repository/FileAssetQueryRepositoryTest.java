...
@DisplayName("FileAssetQueryRepository 단위 테스트")
@ExtendWith(MockitoExtension.class)
class FileAssetQueryRepositoryTest {
    ...
    private FileAssetQueryRepository repository;

    @BeforeEach
    void setUp() {
        repository = new FileAssetQueryRepository(queryFactory);
    }
    ...
    class FindByIdTest {
        ...
        void findById_WhenNotFound_ShouldReturnEmpty() {
            ...
            // when
            Optional<FileAssetJpaEntity> result = repository.findById("not-exist", 100L, 1L);
    ...
    class FindAllTest {
        ...
        void findAll_WithConditions_ShouldReturnEntities() {
            // given
            Long organizationId = 100L;
            Long tenantId = 1L;
            ...
            List<FileAssetJpaEntity> result =
                    repository.findAll(organizationId, tenantId, status, category, offset, limit);
        ...
        void findAll_WhenNoMatch_ShouldReturnEmptyList() {
            // given
            when(queryFactory.selectFrom(fileAssetJpaEntity)).thenReturn(jpaQuery);
            when(jpaQuery.where(any(Predicate[].class))).thenReturn(jpaQuery);
            when(jpaQuery.orderBy(any(OrderSpecifier.class))).thenReturn(jpaQuery);
            ...
            // when
            List<FileAssetJpaEntity> result = repository.findAll(100L, 1L, null, null, 0, 10);
        ...
        void findAll_WithNullFilters_ShouldNotFilter() {
            // given
            List<FileAssetJpaEntity> entities = List.of(createEntity("asset-1", 100L, 1L));
    ...
    class CountTest {
        ...
        void count_WithConditions_ShouldReturnCount() {
            // given
            Long organizationId = 100L;
            Long tenantId = 1L;
            FileAssetStatus status = FileAssetStatus.COMPLETED;
            ...
            // when
            long result = repository.count(organizationId, tenantId, status, category);
        ...
        @Test
        @DisplayName("count 결과가 null이면 0을 반환한다")
        void count_WhenNull_ShouldReturnZero() {
            // given
            when(queryFactory.select(any(NumberExpression.class))).thenReturn(countQuery);
            when(countQuery.from(fileAssetJpaEntity)).thenReturn(countQuery);
            when(countQuery.where(any(Predicate[].class))).thenReturn(countQuery);
            ...
            // when
            long result = repository.count(100L, 1L, null, null);
        ...
        void count_WhenNoMatch_ShouldReturnZero() {
            // given
            when(queryFactory.select(any(NumberExpression.class))).thenReturn(countQuery);
            when(countQuery.from(fileAssetJpaEntity)).thenReturn(countQuery);
            when(countQuery.where(any(Predicate[].class))).thenReturn(countQuery);
            ...
            // when
            long result = repository.count(100L, 1L, FileAssetStatus.PENDING, FileCategory.VIDEO);
