package com.ryuqq.crawlinghub.adapter.out.persistence.architecture.mapper;
 ...
 *
 * <ul>
 *   <li>규칙 1: @Component 어노테이션 필수
 *   <li>규칙 2: Lombok 사용 금지 (6개 어노테이션)
 *   <li>규칙 3: Static 메서드 금지
 *   <li>규칙 4: 비즈니스 로직 금지
 *   <li>규칙 5: toEntity() 메서드 필수
 *   <li>규칙 6: toDomain() 메서드 필수
 *   <li>규칙 7: Mapper 네이밍 규칙 (*Mapper)
 * </ul>
 *
        ...
        return new ArchCondition<>("have public toEntity() method") {
            ...
            public void check(JavaClass javaClass, ConditionEvents events) {
                ...
                if (!hasToEntityMethod) {
                    String message =
                            String.format(
                                    "Class %s does not have a public toEntity() method (required"
                                            + " for Domain → Entity conversion)",
                                    javaClass.getName());
        ...
        return new ArchCondition<>("have public toDomain() method") {
            ...
            public void check(JavaClass javaClass, ConditionEvents events) {
                ...
                if (!hasToDomainMethod) {
                    String message =
                            String.format(
                                    "Class %s does not have a public toDomain() method (required"
                                            + " for Entity → Domain conversion)",
                                    javaClass.getName());
                    events.add(SimpleConditionEvent.violated(javaClass, message));
                }
            }
