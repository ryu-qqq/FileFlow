...
/**
 * External Download Outbox Spring Data JPA Repository
 *
 * <p><strong>역할</strong>: External Download Outbox Entity에 대한 기본 CRUD 및 Command 메서드 제공</p>
 * <p><strong>위치</strong>: adapter-out/persistence-mysql/download/repository/</p>
 * <p><strong>아키텍처</strong>: CQRS Command Side (CUD 작업 전용)</p>
 *
 * <h3>설계 원칙</h3>
 * <ul>
 ...
 *   <li>❌ {@code @Query} 어노테이션 사용 금지 (Query는 QueryAdapter에서)</li>
 * </ul>
 *
 * <h3>Outbox 패턴 Command 쿼리</h3>
 * <ul>
 *   <li>멱등성 키 조회: 중복 이벤트 발행 방지</li>
 *   <li>상태별 조회: PENDING, PROCESSING, COMPLETED, FAILED</li>
 *   <li>완료 건 정리: 오래된 COMPLETED 메시지 삭제</li>
 * </ul>
 *
 * @since 1.0.0
 */
public interface ExternalDownloadOutboxJpaRepository
    ...
    /**
     * 특정 시간 이전에 생성된 상태별 Outbox 메시지 조회
     *
     * <p>특정 상태이면서 특정 시간 이전에 생성된 Outbox 메시지를 조회합니다.</p>
    ...
    /**
     * 특정 시간 이전에 생성된 상태별 Outbox 메시지 삭제
     *
     * <p>특정 상태이면서 특정 시간 이전에 생성된 Outbox 메시지를 삭제합니다.</p>
     * <p>오래된 완료 건 정리(Cleanup) 시 사용합니다.</p>
     *
     * @param status Outbox 상태
     * @param createdBefore 생성 시간 기준
