...
public class OrganizationPersistenceAdapter implements OrganizationRepositoryPort {
     ...
     * @return Organization Domain 목록 (빈 리스트 가능)
     ...
     *
     * <p>특정 Tenant 내에서 조직 코드로 Organization을 찾습니다.</p>
     *
     * @param tenantId 조회할 Tenant ID (Long FK)
     ...
     */
    @Override
    public Optional<Organization> findByTenantIdAndOrgCode(Long tenantId, OrgCode orgCode) {
        if (tenantId == null) {
            throw new IllegalArgumentException("tenantId must not be null");
        }
        if (orgCode == null) {
     ...
     */
    @Override
    public boolean existsByTenantIdAndOrgCode(Long tenantId, OrgCode orgCode) {
        if (tenantId == null) {
            throw new IllegalArgumentException("tenantId must not be null");
        }
        if (orgCode == null) {
