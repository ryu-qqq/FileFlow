...
public interface TransformQueueOutboxJpaRepository
    ...
    @Query(
            value =
                    "UPDATE transform_queue_outbox SET outbox_status = 'PROCESSING', processed_at ="
    ...
    @Query("SELECT e FROM TransformQueueOutboxJpaEntity e WHERE e.outboxStatus = :status")
    List<TransformQueueOutboxJpaEntity> findByStatus(@Param("status") OutboxStatus status);
    ...
    @Query(
            value =
                    "UPDATE transform_queue_outbox SET retry_count = retry_count + 1, processed_at"
                        + " = :now, outbox_status = CASE WHEN retry_count + 1 >= 5 THEN 'FAILED'"
