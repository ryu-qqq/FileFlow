...
output "endpoint" {
  description = "The connection endpoint (address:port or configuration endpoint)"
  value       = var.replication_group_id != null ? "${aws_elasticache_replication_group.redis[0].primary_endpoint_address}:${local.port}" : (var.engine == "memcached" ? aws_elasticache_cluster.memcached-or-single-redis[0].configuration_endpoint : "${aws_elasticache_cluster.memcached-or-single-redis[0].cache_nodes[0].address}:${local.port}")
}

output "endpoint_address" {
  description = "The connection endpoint address only (without port)"
  value       = var.replication_group_id != null ? aws_elasticache_replication_group.redis[0].primary_endpoint_address : (var.engine == "memcached" ? split(":", aws_elasticache_cluster.memcached-or-single-redis[0].configuration_endpoint)[0] : aws_elasticache_cluster.memcached-or-single-redis[0].cache_nodes[0].address)
}
