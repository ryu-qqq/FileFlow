package com.ryuqq.fileflow.application.iam.usercontext.usecase;

import com.ryuqq.fileflow.application.iam.usercontext.dto.AssignRoleCommand;

/**
 * Role 할당 UseCase 인터페이스
 *
 * <p>사용자에게 새로운 Role(Membership)을 할당하는 비즈니스 유스케이스입니다.</p>
 ...
 *   <li>✅ 도메인 이벤트 발행 (RoleAssignedEvent)</li>
 * </ul>
 *
 ...
 * <pre>
 * AssignRoleUseCase
 *   1. Command → Domain 변환 (Membership 생성)
 *   2. UserContext 조회 (Repository)
 *   3. userContext.addMembership(membership)  ← Domain 로직
 ...
 *   <li>{@code IllegalStateException}: UserContext가 삭제됨 또는 중복 Membership 존재</li>
 ...
 *
 * AssignRoleCommand command = AssignRoleCommand.of(
 *     123L,           // userId
 *     "tenant-1",     // tenantId
 *     456L,           // organizationId
 ...
 * // → UserContext에 Membership 추가 + 캐시 무효화 + 이벤트 발행
 * </pre>
 *
 * @author ryu-qqq
 * @since 2025-10-26
