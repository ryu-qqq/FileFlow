package com.ryuqq.fileflow.application.architecture.dto;
 ...
 *
 * <p>모든 DTO (Command, Query, Response)는 정확히 이 규칙을 따라야 합니다:
 *
 * <ul>
 *   <li>Command: dto/command/ 패키지에 Record 타입으로 정의
 *   <li>Query: dto/query/ 패키지에 Record 타입으로 정의
 *   <li>Response: dto/response/ 패키지에 Record 타입으로 정의
 *   <li>Lombok 절대 금지 (Record 사용)
 *   <li>jakarta.validation 의존성 금지 (순수 Java Record)
 *   <li>비즈니스 로직 금지 (데이터 전달만)
    ...
    void dto_MustNotUseJakartaValidation() {
        ArchRule rule =
                noClasses()
                        ...
                        .because(
                                "DTO는 순수 Java Record를 사용해야 합니다 (jakarta.validation 금지, REST API"
                                        + " Layer에서 검증)");
