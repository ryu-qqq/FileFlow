...
class DownloadCommandFactoryTest {
    ...
    void setUp() {
        ...
        sut =
                new DownloadCommandFactory(
                        idGeneratorPort, timeProvider, assetCommandFactory, storageBucketManager);
    }
    ...
    class CreateTest {
        ...
        @Test
        @DisplayName("커맨드로 QUEUED 상태의 DownloadTask를 생성하며 s3Key를 자동 생성한다")
        void create_ValidCommand_ReturnsQueuedDownloadTaskWithAutoGeneratedS3Key() {
            // given
            given(idGeneratorPort.generate()).willReturn("download-001");
            given(storageBucketManager.getBucket()).willReturn("test-bucket");
            ...
            assertThat(result.s3Key()).isEqualTo("public/2026/01/download-001.jpg");
        ...
        @Test
        @DisplayName("확장자가 없는 URL에서는 확장자 없이 s3Key를 생성한다")
        void create_UrlWithoutExtension_GeneratesS3KeyWithoutExtension() {
            // given
            given(idGeneratorPort.generate()).willReturn("download-002");
            given(storageBucketManager.getBucket()).willReturn("test-bucket");

            CreateDownloadTaskCommand command =
                    new CreateDownloadTaskCommand(
                            "https://example.com/files/noextension",
                            AccessType.INTERNAL,
                            "document",
                            "doc-service",
                            null);
            ...
            // then
            assertThat(result.s3Key()).isEqualTo("internal/2026/01/download-002");
            assertThat(result.bucket()).isEqualTo("test-bucket");
            assertThat(result.accessType()).isEqualTo(AccessType.INTERNAL);
        }
    }
