...
/**
 * Aggregate ID Value Object (Record 구현)
 * <p>
 * 모든 Aggregate의 aggregateId 필드에 사용되는 공통 식별자입니다.
...
public record AggregateId(String value) {
    ...
    public AggregateId {
        if (value != null && value.isBlank()) {
            throw new IllegalArgumentException("Aggregate ID는 빈 값일 수 없습니다");
        }
    }

    /**
     * AggregateId 정적 팩토리 메서드
     * <p>
     * non-null 값으로 AggregateId를 생성합니다.
     * </p>
     *
     * @param value ID 값 (null 불가)
     * @return AggregateId 인스턴스
     * @throws IllegalArgumentException ID가 null이거나 blank일 때
     */
    public static AggregateId of(String value) {
        if (value == null) {
            throw new IllegalArgumentException("Aggregate ID는 null이거나 빈 값일 수 없습니다");
        }
        return new AggregateId(value);
    }

    /**
     * 신규 Aggregate용 AggregateId 생성
     * <p>
     * 영속화 전 상태를 나타내기 위해 null 값을 가진 AggregateId를 반환합니다.
     * </p>
     *
     * @return null 값을 가진 AggregateId
     */
    public static AggregateId forNew() {
        return new AggregateId(null);
    }
