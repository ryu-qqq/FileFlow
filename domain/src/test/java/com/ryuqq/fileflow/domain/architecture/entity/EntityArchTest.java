package com.ryuqq.crawlinghub.domain.architecture.entity;
 ...
 *
 * <ul>
 *   <li>Entity 인터페이스 구현 필수 (AggregateRoot 아님)
 *   <li>ID 기반 equals/hashCode
 *   <li>생성자 private
 *   <li>Factory 메서드 (forNew, reconstitute)
 *   <li>domain.[bc].aggregate 패키지에 위치
 *   <li>Repository 없음 (AggregateRoot만 Repository 가능)
 *   <li>Lombok, JPA, Spring 금지
 * </ul>
 *
 * <p><strong>Entity vs AggregateRoot 구분</strong>:
 *
 * <ul>
 *   <li>AggregateRoot: 외부 접근 가능, Repository 존재, 트랜잭션 경계
 *   <li>Entity (내부): AggregateRoot를 통해서만 접근, Repository 없음
 * </ul>
 *
 * <p><strong>예시</strong>:
 *
...
class EntityArchTest {
    ...
    void internalEntities_ShouldImplementEntityInterface() {
        ArchRule rule =
                classes()
                        .that()
                        .implement("com.ryuqq.authhub.domain.common.model.Entity")
                        .and()
                        .doNotImplement("com.ryuqq.authhub.domain.common.model.AggregateRoot")
                        .and()
                        ...
                        .because(
                                "내부 Entity는 Entity 인터페이스를 구현해야 하며, AggregateRoot는 구현하지 않아야 합니다\n"
                                        + "예시:\n"
                                        + "  - OrderItem implements Entity ✅\n"
                                        + "  - OrderItem implements AggregateRoot ❌ (내부 Entity는"
                                        + " AggregateRoot 아님)");
    ...
    void entities_ShouldHaveEqualsAndHashCode() {
        ArchRule rule =
                classes()
                        ...
                        .because(
                                "Entity는 ID 기반 equals()와 hashCode()를 구현해야 합니다\n"
                                    + "예시:\n"
                                    + "  @Override\n"
    ...
    void entities_ShouldHavePrivateConstructors() {
        ArchRule rule =
                classes()
                        ...
                        .because(
                                "Entity는 Factory Method를 통해서만 생성되어야 하므로 생성자는 private이어야 합니다\n"
                                        + "예시:\n"
                                        + "  private OrderItem(...) { }  ✅\n"
                                        + "  public OrderItem(...) { }   ❌");
    ...
    void entities_ShouldHaveForNewMethod() {
        ArchRule rule =
                classes()
                        ...
                        .because(
                                "Entity는 forNew() 정적 팩토리 메서드를 가져야 합니다\n"
                                        + "예시:\n"
                                        + "  public static OrderItem forNew(long productId, int"
    ...
    void entities_ShouldHaveReconstituteMethod() {
        ArchRule rule =
                classes()
                        ...
                        .because(
                                "Entity는 reconstitute() 정적 팩토리 메서드를 가져야 합니다\n"
                                        + "예시:\n"
                                        + "  public static OrderItem reconstitute(\n"
                                        + "      OrderItemId id,\n"
                                        + "      long productId,\n"
    ...
    void entities_ShouldBeInAggregatePackage() {
        ArchRule rule =
                classes()
                        ...
                        .because(
                                "내부 Entity는 domain.[bc].aggregate 패키지에 위치해야 합니다\n"
                                    + "예시:\n"
    ...
    void entities_ShouldUseFieldNameMethodsInsteadOfGetters() {
        ...
        ArchRule rule =
                classes()
                        ...
                        .because(
                                "Entity는 get* 메서드 대신 필드명 메서드를 사용해야 합니다 (Tell, Don't Ask)\n"
                                        + "예시:\n"
    ...
    private static ArchCondition<JavaClass> implementEntityInterfaceOnly() {
        return new ArchCondition<JavaClass>("implement Entity interface only (not AggregateRoot)") {
            ...
            public void check(JavaClass javaClass, ConditionEvents events) {
                ...
                boolean implementsAggregateRoot =
                        javaClass.getAllRawInterfaces().stream()
                ...
                if (!implementsEntity || implementsAggregateRoot) {
                    String message =
                            String.format(
                                    "Class %s should implement Entity interface only (not"
                                            + " AggregateRoot)",
                                    javaClass.getName());
        ...
        return new ArchCondition<JavaClass>("have equals() and hashCode() methods") {
            ...
            public void check(JavaClass javaClass, ConditionEvents events) {
                ...
                if (!hasEquals || !hasHashCode) {
                    String message =
                            String.format(
                                    "Class %s should have equals() and hashCode() methods"
                                            + " (ID-based)",
                                    javaClass.getName());
                    events.add(SimpleConditionEvent.violated(javaClass, message));
                }
        ...
        return new ArchCondition<JavaClass>("have only private constructors") {
            ...
            public void check(JavaClass javaClass, ConditionEvents events) {
                ...
                if (hasPublicConstructor) {
                    String message =
                            String.format(
                                    "Class %s should have only private constructors (use Factory"
                                            + " Methods: forNew, reconstitute)",
                                    javaClass.getName());
                    events.add(SimpleConditionEvent.violated(javaClass, message));
                }
            }
        ...
        return new ArchCondition<JavaClass>("prefer field name methods over getters") {
            ...
            public void check(JavaClass javaClass, ConditionEvents events) {
                ...
                if (getterCount > 0) {
                    String message =
                            String.format(
                                    "Class %s has %d getter methods (get*). Consider using field"
                                            + " name methods instead (e.g., price() instead of"
                                            + " getPrice())",
