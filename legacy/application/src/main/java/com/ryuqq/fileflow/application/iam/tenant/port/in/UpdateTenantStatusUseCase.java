package com.ryuqq.fileflow.application.iam.tenant.port.in;

import com.ryuqq.fileflow.application.iam.tenant.dto.command.UpdateTenantStatusCommand;
import com.ryuqq.fileflow.application.iam.tenant.dto.response.TenantResponse;

/**
 * UpdateTenantStatusUseCase - Tenant 상태 변경 UseCase (Inbound Port)
 *
 * <p>Hexagonal Architecture의 Driving Port (Application Layer → Domain Layer)입니다.</p>
 *
 * <p><strong>책임:</strong></p>
 * <ul>
 *   <li>Tenant의 상태를 변경합니다 (ACTIVE ↔ SUSPENDED)</li>
 *   <li>상태 변경 후 변경된 Tenant 정보를 반환합니다</li>
 * </ul>
 *
 * <p><strong>Port 설계 원칙:</strong></p>
 * <ul>
 *   <li>✅ 단일 책임 원칙 (SRP) - 상태 변경만 담당</li>
 *   <li>✅ Interface Segregation - 다른 UseCase와 분리</li>
 *   <li>✅ Command 기반 입력 (UpdateTenantStatusCommand)</li>
 *   <li>✅ Response 기반 출력 (TenantResponse)</li>
 * </ul>
 *
 * <p><strong>상태 전환 규칙:</strong></p>
 * <ul>
 *   <li>ACTIVE → SUSPENDED: Tenant 일시 정지</li>
 *   <li>SUSPENDED → ACTIVE: Tenant 재활성화</li>
 * </ul>
 *
 * <p><strong>예외 처리:</strong></p>
 * <ul>
 *   <li>TenantNotFoundException: Tenant가 존재하지 않는 경우</li>
 *   <li>IllegalArgumentException: 잘못된 상태값</li>
 *   <li>InvalidStatusTransitionException: 허용되지 않는 상태 전환</li>
 * </ul>
 *
 * <p><strong>사용 예시:</strong></p>
 * <pre>{@code
 * UpdateTenantStatusCommand command = new UpdateTenantStatusCommand("tenant-id-123", "SUSPENDED");
 * TenantResponse response = updateTenantStatusUseCase.execute(command);
 * }</pre>
 *
 * @author ryu-qqq
 * @since 2025-10-23
 */
public interface UpdateTenantStatusUseCase {

    /**
     * Tenant 상태 변경
     *
     * @param command Tenant 상태 변경 Command
     * @return 변경된 Tenant 정보
     * @throws com.ryuqq.fileflow.domain.iam.tenant.exception.TenantNotFoundException Tenant가 존재하지 않는 경우
     * @throws IllegalArgumentException 잘못된 상태값
     * @author ryu-qqq
     * @since 2025-10-23
     */
    TenantResponse execute(UpdateTenantStatusCommand command);
}
