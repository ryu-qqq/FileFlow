...
> **반드시 첫 단계에서 activate_project 실행 → 메모리 복원 → 규칙 인덱스 캐싱 → 컨텍스트 요약 반환**

---

## 실행 워크플로우

### Phase 1: Serena 프로젝트 활성화 (필수)

```python
# 1. 프로젝트 활성화 (반드시 가장 먼저 실행)
activate_project(directory="/Users/sangwon-ryu/fileflow")
# 현재 작업 디렉토리를 Serena에 등록하여 메모리 컨텍스트 활성화

# 2. 기존 메모리 목록 조회
memories = list_memories()
```

**주의**: `activate_project`는 모든 Serena MCP 작업의 전제 조건입니다.
이 단계를 건너뛰면 메모리 읽기/쓰기가 실패합니다.
...
try:
    ...
    return """
    ⚠️ Serena 프로젝트 활성화 실패

    원인: {e}

    해결 방법:
    1. Serena MCP 서버가 실행 중인지 확인
    2. 프로젝트 경로가 올바른지 확인
    3. /load 명령을 다시 실행
    """
```
...
```python
# 메모리 복원 실패는 경고만 표시하고 계속 진행
if not memories:
    warning = "⚠️ 이전 세션 메모리 없음 (새 세션)"
```
