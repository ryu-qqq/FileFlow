...
terraform {
  ...
  backend "s3" {
    ...
    key            = "fileflow/ecs-download-worker-stage/terraform.tfstate"
...
variable "image_tag" {
  description = "Docker image tag to deploy. Auto-set by GitHub Actions build-and-deploy.yml. Format: download-worker-stage-{build-number}-{git-sha}"
  type        = string
  default     = "download-worker-stage-1-initial"

  validation {
    condition     = can(regex("^download-worker-stage-[0-9]+-[a-zA-Z0-9]+$", var.image_tag))
    error_message = "Image tag must follow format: download-worker-stage-{build-number}-{git-sha} (e.g., download-worker-stage-1-abc123)"
  }
}
