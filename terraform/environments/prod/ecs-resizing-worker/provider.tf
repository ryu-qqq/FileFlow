...
terraform {
  ...
  backend "s3" {
    ...
    key            = "fileflow/ecs-resizing-worker/terraform.tfstate"
...
variable "image_tag" {
  ...
  type        = string
  default     = "resizing-worker-1-0000000"  # Fallback only - GitHub Actions will override this

  validation {
    condition     = can(regex("^resizing-worker-[0-9]+-[a-f0-9]+$", var.image_tag))
    error_message = "Image tag must follow format: resizing-worker-{build-number}-{git-sha} (e.g., resizing-worker-92-f08d571)"
  }
}
