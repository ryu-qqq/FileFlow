 ...
 * 통합 테스트용 데이터 삽입 헬퍼.
 *
 * <p>JdbcTemplate을 사용하여 테스트 데이터를 직접 삽입합니다. Domain/Application 레이어를 우회하여 빠르게 테스트 데이터를 설정할 수 있습니다.
 *
 * <p>주의사항: - 이 클래스는 통합 테스트에서만 사용해야 합니다. - 테이블 구조가 변경되면 이 클래스도 업데이트해야 합니다.
 */
@Component
public class TestDataHelper {
    ...
    /** SingleUploadSession 테스트 데이터를 삽입합니다. 현재 스키마에 맞게 필수 컬럼들을 모두 설정합니다. */
    ...
    public Long insertProcessedFileAsset(
        ...
        LocalDateTime now = LocalDateTime.now();
        jdbcTemplate.update(
                sql, fileAssetId, variantType, storedFileName, width, height, fileSize, now, now);
    ...
    /**
     * ExternalDownload 테스트 데이터를 삽입합니다. 현재 스키마에 맞게 필수 컬럼들을 모두 설정합니다.
     *
    ...
    public String insertExternalDownload(
        ...
        jdbcTemplate.update(
                ...
                tenantId,
                organizationId,
                "fileflow-uploads-prod",
                "external-downloads/" + tenantId,
                status,
                0,
                null,
                null,
    ...
    public String insertExternalDownload(String sourceUrl) {
        return insertExternalDownload(
                sourceUrl, UUID.randomUUID().toString(), UUID.randomUUID().toString());
    }
