...
@DisplayName("MultipartSessionZombieRecoveryScheduler 단위 테스트")
class MultipartSessionZombieRecoverySchedulerTest {

    private MultipartSessionZombieRecoveryScheduler sut;

    @Mock private RecoverExpiredMultipartSessionUseCase useCase;

    private static final int BATCH_SIZE = 50;

    @BeforeEach
    void setUp() {
        SchedulerProperties.MultipartSessionZombieRecovery config =
                new SchedulerProperties.MultipartSessionZombieRecovery(
                        true, "0 */5 * * * *", "Asia/Seoul", BATCH_SIZE);
        SchedulerProperties.Jobs jobs =
                new SchedulerProperties.Jobs(null, null, null, null, null, null, null, config);
