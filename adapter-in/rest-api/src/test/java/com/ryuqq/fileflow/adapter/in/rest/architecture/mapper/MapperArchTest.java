package com.ryuqq.fileflow.adapter.in.rest.architecture.mapper;
 ...
 *   <li>✅ *ApiMapper 네이밍 규칙
 *   <li>❌ Lombok 어노테이션 절대 금지
 *   <li>❌ Static 메서드 절대 금지
 *   <li>❌ Domain 객체 직접 사용 금지
 *   <li>❌ 비즈니스 로직 메서드 금지
 *   <li>❌ Port 의존성 주입 금지
 *   <li>✅ 올바른 패키지 위치
 * </ul>
...
class MapperArchTest {
    ...
    /**
     * 규칙 5: Domain Aggregate/Service 직접 사용 금지 (ErrorMapper 제외)
     *
     * <p>Domain의 Value Object(enum, VO)는 데이터 표현 목적으로 허용됩니다. Domain Aggregate, Service 등의 핵심 로직 클래스만
     * 의존 금지합니다.
     */
    @Test
    @DisplayName("[금지] Mapper는 Domain Aggregate/Service를 직접 사용하지 않아야 한다 (ErrorMapper 제외)")
    void mapper_MustNotUseDomainAggregateOrService() {
        ArchRule rule =
                noClasses()
                        .that()
                        .resideInAPackage("..mapper..")
                        ...
                        .because(
                                "Mapper는 Application DTO만 사용하며 Domain Aggregate/Service 직접 의존은"
                                        + " 금지됩니다 (ErrorMapper는 예외, Domain VO/enum은 허용)");
