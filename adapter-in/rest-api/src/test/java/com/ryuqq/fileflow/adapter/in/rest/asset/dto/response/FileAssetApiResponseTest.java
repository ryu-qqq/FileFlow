...
class FileAssetApiResponseTest {
    ...
    class CreateTest {
        ...
        void create_WithAllFields_ShouldSucceed() {
            ...
            assertThat(response.status()).isEqualTo(FileAssetStatus.COMPLETED);
        ...
        void create_WithNullProcessedAt_ShouldSucceed() {
            ...
            // then
            assertThat(response.status()).isEqualTo(FileAssetStatus.PENDING);
    ...
    class CategoryTest {
        ...
        void create_WithAllCategories_ShouldSucceed(FileCategory category) {
            // when
            FileAssetApiResponse response =
                    new FileAssetApiResponse(
                            "id",
    ...
    class StatusTest {
        ...
        void create_WithAllStatuses_ShouldSucceed(FileAssetStatus status) {
            // when
            FileAssetApiResponse response =
                    new FileAssetApiResponse(
                            "id",
                            ...
                            "bucket",
                            "key",
                            status == FileAssetStatus.COMPLETED ? "etag" : null,
                            status,
                            LocalDateTime.now(),
                            status == FileAssetStatus.COMPLETED ? LocalDateTime.now() : null);
    ...
    class ContentTypeTest {
        ...
        void create_WithImageContentType_ShouldSucceed() {
            // when
            FileAssetApiResponse response =
                    createResponse("image.png", "image/png", FileCategory.IMAGE);

            // then
            assertThat(response.contentType()).isEqualTo("image/png");
            assertThat(response.category()).isEqualTo(FileCategory.IMAGE);
        }
        ...
        void create_WithVideoContentType_ShouldSucceed() {
            // when
            FileAssetApiResponse response =
                    createResponse("video.mp4", "video/mp4", FileCategory.VIDEO);

            // then
            assertThat(response.contentType()).isEqualTo("video/mp4");
            assertThat(response.category()).isEqualTo(FileCategory.VIDEO);
        }
        ...
        void create_WithAudioContentType_ShouldSucceed() {
            // when
            FileAssetApiResponse response =
                    createResponse("audio.mp3", "audio/mpeg", FileCategory.AUDIO);

            // then
            assertThat(response.contentType()).isEqualTo("audio/mpeg");
            assertThat(response.category()).isEqualTo(FileCategory.AUDIO);
        }
        ...
        void create_WithDocumentContentType_ShouldSucceed() {
            // when
            FileAssetApiResponse response =
                    createResponse("doc.pdf", "application/pdf", FileCategory.DOCUMENT);
    ...
    private FileAssetApiResponse createFullResponse(String id, LocalDateTime now) {
        return new FileAssetApiResponse(
                id,
                "session",
                "file.jpg",
