...
class ApiResponseArchTest {
     ...
     *
     * <p>ApiResponse, PageApiResponse, SliceApiResponse, ErrorInfo 등 공통 응답 DTO만 검증합니다. BC별 응답
     * DTO(*ApiResponse)는 response 패키지에 위치해야 하므로 별도 검증합니다.
     */
    @Test
    @DisplayName("[필수] Common Response DTOs는 common.dto 패키지에 위치해야 한다")
    void commonResponseDtos_MustBeInCommonDtoPackage() {
        // ApiResponse 검증
        ArchRule apiResponseRule =
                classes()
                        .that()
                        ...
                        .because("ApiResponse는 common.dto 패키지에 위치해야 합니다");

        // PageApiResponse 검증
        ArchRule pageApiResponseRule =
                classes()
                        .that()
                        ...
                        .should()
                        .resideInAPackage("..common.dto..")
                        .because("PageApiResponse는 common.dto 패키지에 위치해야 합니다");

        // SliceApiResponse 검증
        ArchRule sliceApiResponseRule =
                classes()
                        .that()
                        ...
                        .should()
                        .resideInAPackage("..common.dto..")
                        .because("SliceApiResponse는 common.dto 패키지에 위치해야 합니다");

        // ErrorInfo 검증
        ArchRule errorInfoRule =
                classes()
                        .that()
                        ...
                        .should()
                        .resideInAPackage("..common.dto..")
                        .because("ErrorInfo는 common.dto 패키지에 위치해야 합니다");

        apiResponseRule.allowEmptyShould(true).check(classes);
        pageApiResponseRule.allowEmptyShould(true).check(classes);
        sliceApiResponseRule.allowEmptyShould(true).check(classes);
        errorInfoRule.allowEmptyShould(true).check(classes);
    }
