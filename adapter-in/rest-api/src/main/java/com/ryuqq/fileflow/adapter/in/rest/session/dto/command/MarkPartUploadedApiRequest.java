...
public record MarkPartUploadedApiRequest(
        @Schema(description = "Part 번호 (1-based)", example = "1", requiredMode = Schema.RequiredMode.REQUIRED)
        @Positive(message = "Part 번호는 양수여야 합니다") int partNumber,
        @Schema(description = "S3가 반환한 Part ETag", example = "\"d41d8cd98f00b204e9800998ecf8427e\"", requiredMode = Schema.RequiredMode.REQUIRED)
        @NotBlank(message = "ETag는 필수입니다") String etag,
        @Schema(description = "Part 크기 (bytes)", example = "5242880", requiredMode = Schema.RequiredMode.REQUIRED)
