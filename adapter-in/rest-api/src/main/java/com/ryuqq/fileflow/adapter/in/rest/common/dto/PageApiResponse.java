...
public record PageApiResponse<T>(
     ...
     *
     * <p>Application Layer의 PageResponse를 Adapter-In Layer의 PageApiResponse로 변환합니다.
     *
     ...
     *
     * <p>Application Layer의 PageResponse를 Adapter-In Layer의 PageApiResponse로 변환하면서,
     *
    ...
    public static <S, T> PageApiResponse<T> from(
        ...
        List<T> content = appPageResponse.content().stream().map(mapper).toList();
